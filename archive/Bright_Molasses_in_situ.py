...
if shot_globals.do_tweezers:
    ...
    devices.tweezer_aom_analog.constant(t, 0.01) #0.3) #for single tweezer


t += 2 #0.5  # how long MOT last
...
if shot_globals.do_blow_beam:
    ...
    devices.ta_aom_analog.constant(t , 0.15) # change power for blowing
    # devices.ta_aom_digital.go_high(t)
    devices.repump_aom_digital.go_high(t)

    t += shot_globals.blow_beam_time
    ...
    devices.ta_vco.ramp(
            t,
            duration=ta_vco_ramp_t,
            initial=ta_freq_calib(ta_bm_detuning), #mot_detuning
            final=ta_freq_calib(shot_globals.ta_img_detuning),
            samplerate=4e5,
        )# ramp back to imaging
...
# if shot_globals.do_dipole_trap_camera:
#     devices.manta419b_dipole_trap.expose(
#         'manta419b',
#         t,
#         'atoms',
#         exposure_time=max(shot_globals.manta_exposure, 50e-6),
