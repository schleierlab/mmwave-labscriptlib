...
bipolar_coil_flip_time = 10e-3 # the time takes to flip the polarity of the coil
coil_feedback_off_time = 4.5e-3 # how long to turn off the feedback of circuit when flipping polarity
...
def do_imaging(t, shot_number, ta_last_detuning, repump_last_detuning, exposure = shot_globals.bm_exposure_time):
    ...
    if shot_number ==1:
        ...
        if shot_globals.do_shutter_close_after_first_shot:
            if shot_globals.do_mot_beams_during_imaging:
                if shot_globals.do_mot_xy_beams_during_imaging:
                    devices.mot_xy_shutter.open(t)
                if shot_globals.do_mot_z_beam_during_imaging:
...
def x_coil_flip_polarity(t, x_ini, x_fin):
    coil_voltage_mid_abs = 0.03
    coil_voltage_mid = np.sign(biasx_calib(x_fin)) * coil_voltage_mid_abs
...
def y_coil_flip_polarity(t, y_ini, y_fin):
    coil_voltage_mid_abs = 0.03
    coil_voltage_mid = np.sign(biasy_calib(y_fin)) * coil_voltage_mid_abs
    total_coil_flip_ramp_time = bipolar_coil_flip_time + coil_ramp_time
...
def z_coil_flip_polarity(t, z_ini, z_fin):
    coil_voltage_mid_abs = 0.03
    coil_voltage_mid = np.sign(biasz_calib(z_fin)) * coil_voltage_mid_abs
    total_coil_flip_ramp_time = bipolar_coil_flip_time + coil_ramp_time
...
def coil_change(t, x_ini, x_fin, y_ini, y_fin, z_ini, z_fin):
    if np.sign(biasx_calib(x_fin)*biasx_calib(x_ini)) > 0:
        devices.x_coil_current.ramp(
                t,
                duration=coil_ramp_time,
                initial=biasx_calib(x_ini),
                final=biasx_calib(x_fin),
                samplerate=1e5,
        )
    else: # coil flip the control voltage sign
        x_coil_flip_polarity(t, x_ini, x_fin)
    ...
    if np.sign(biasy_calib(y_fin)*biasy_calib(y_ini)) > 0:  # coil flip the control voltage sign
        ...
        print('flip bias coil y polarity!')
        y_coil_flip_polarity(t, y_ini, y_fin)
    ...
    if np.sign(biasz_calib(z_fin)*biasz_calib(z_ini)) > 0:  # coil flip the control voltage sign
        ...
        z_coil_flip_polarity(t, z_ini, z_fin)
