...
from calibration import ta_freq_calib, repump_freq_calib, biasx_calib, biasy_calib, biasz_calib, tweezer_power_calib
...
tweezer_aom_thermal_time = 10e-6 #time it takes for tweezer power to stablize when change from 1 V to 0.15 V
...
def robust_loading_pulse(t, dur):
    ...
    devices.ta_vco.ramp(
        ...
        duration=ta_vco_ramp_t,
        initial=ta_freq_calib(ta_last_detuning),
        final=ta_freq_calib(shot_globals.robust_loading_ta_detuning),
        samplerate=4e5,
    )# ramp back to imaging
    ...
    ta_last_detuning = shot_globals.robust_loading_ta_detuning
    repump_last_detuning = 0

    devices.ta_aom_analog.constant(t , shot_globals.robust_loading_ta_power) # back to full power for imaging
    devices.repump_aom_analog.constant(t , shot_globals.robust_loading_repump_power)
...
def depump_pulse(t):
    ...
    devices.optical_pump_shutter.open(t)
    devices.ta_aom_digital.go_high(t)
    devices.ta_aom_analog.constant(t, 0.01)
    devices.tweezer_aom_analog.constant(t-5e-6, shot_globals.tw_depump_pulse_power)
...
t = 0
devices.dds1.synthesize(1e-4, shot_globals.blue_456_detuning , 0.2, 0)
devices.moglabs_456_aom_analog.constant(t, shot_globals.blue_456nm_power)
devices.moglabs_456_aom_digital.go_high(t)
devices.blue_456_shutter.close(t)
devices.ipg_1064_aom_analog.constant(t,shot_globals.ipg_1064_power)
devices.ipg_1064_aom_digital.go_low(t)


devices.mirror_1_vertical.constant(t, shot_globals.piezo_mirror_1_vertical)
devices.mirror_2_vertical.constant(t, shot_globals.piezo_mirror_2_vertical)
devices.mirror_1_horizontal.constant(t, shot_globals.piezo_mirror_1_horizontal)
devices.mirror_2_horizontal.constant(t, shot_globals.piezo_mirror_2_horizontal)
...
#     t += 7e-3 #for shutter to close and open after optical pump before killing pulse
...
if shot_globals.do_rydberg_beams:
    ...
    if shot_globals.do_rydberg_pulse_with_repump:
        devices.optical_pump_shutter.open(t)
        devices.repump_shutter.open(t)
    devices.moglabs_456_aom_digital.go_low(t-3e-3)
    if not(shot_globals.do_rydberg_pulse_1064_off):
